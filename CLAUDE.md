...
3. **When done**, and only when no remaining work is left:
   ...
   - Clean up the worktree (`git worktree remove ...`) and delete the
     working branch.
4. **Always push as you go** — after every merge to `main` (and any
   other commit that lands on `main`), push to `origin main`
   immediately. Do not accumulate unpushed commits or wait to be
   asked. (CLAUDE.md itself is the exception: it must not be
   committed to `main` — sync it via `scripts/claude.sh push`
   instead, per the "Shared LLM context branch" section below.)
