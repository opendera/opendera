...
```bash
RUST_BACKTRACE=1 RUST_LOG=warn,pipeline_manager=info,opendera_types=info,project=info,dbsp=info,dbsp_adapters=info,dbsp_nexmark=info cargo run --package=pipeline-manager --bin pipeline-manager -- --dev-mode
```
